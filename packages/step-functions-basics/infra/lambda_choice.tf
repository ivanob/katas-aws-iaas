...
resource "aws_lambda_function" "lambda_choice" {
  filename         = data.archive_file.zip.output_path
  function_name    = "handler_choice"
  handler          = "lambda-choice.handler"
  runtime          = "nodejs16.x"
...
data "archive_file" "zip" {
  type        = "zip"
  source_file = "../dist/lambda-choice.js"
  output_path = "../dist/lambda-choice.zip"
}

resource "aws_iam_role" "iam_for_lambda" {
  name               = "lambda_role_choice"
